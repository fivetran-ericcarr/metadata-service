...
from .engine import FAIL, PASS, SKIPPED, WAIVED, Policy, evaluate, gate_activation
...
def evaluate_cmd(
    ...
    policy: str = typer.Option("policy.toml", "--policy"),
    api_key: str | None = typer.Option(None, "--api-key", envvar="METADATA_API_KEY"),
    as_json: bool = typer.Option(False, "--json", help="Emit the full report as JSON."),
) -> None:
    """Evaluate the policy against the snapshot. Exit 0 on pass, 1 on fail."""
...
def gate_activation_cmd(
    ...
    """Pre-flight one reverse-ETL sync. Exit 0 = allow, 1 = deny (fail closed)."""
    doc, pol = _load(snapshot, policy, api_key)
...
# typer needs distinct python names; expose `evaluate` as the command name.
app.registered_commands[0].name = "evaluate"
